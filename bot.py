    ...
    try:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"⚠️ CoinGecko Rate Limited ({resp.status_code}). Using MEXC volume fallback...", flush=True)
        except Exception as cg_err:
            print(f"⚠️ CoinGecko API error: {cg_err}. Falling back to MEXC volume scan...", flush=True)
        ...
        for symbol in mexc_exchange.symbols:
            if symbol.endswith(":USDT") and "UP" not in symbol and "DOWN" not in symbol:
                base_currency = symbol.split("/")[0]  # Extract coin name (e.g., 'BTC')
                
                if high_vol_coins:
                    if base_currency in high_vol_coins:
                        matched_symbols.append(symbol)
                else:
                    # Backup check directly on MEXC
